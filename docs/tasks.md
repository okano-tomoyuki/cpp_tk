...
初回はWidget共通機能・classic OptionMenu/LabelFrame・Menu/Listbox/Text/filedialog/messageboxのみが対象で、Canvas/ttkの細部やToplevel周辺は未着手だった。実利用頻度が高い順に対応し、[test/test_tkinter_parity_batch2.cpp](../test/test_tkinter_parity_batch2.cpp)で全項目を回帰テスト化した(11 TEST_CASE、59 assertion、全green)。
...
- **生成時専用(read-only)オプションを設定する手段が無い**: `ttk::panedwindow`の`-orient`のように「生成時にしか指定できないオプション」を持つウィジェットが存在するが、cpp_tkの`Widget(parent, type, name)`は常に「(1)オプション無しで生成 → (2)`configure`で別途設定」という2段階の実装になっているため、こうしたオプションを設定する手段が現状無い(`ttk::PanedWindow::orient()`を生成後に呼ぶと`attempt to change read-only option`でErrorになる。動作確認用テストで実際に踏んだため、既定値のままテストするよう回避した)。
  ...
  **対応内容**: `Widget(parent, type, name, options={})`という4引数版を追加し、`options`を生成コマンド自体に埋め込む(`call({type, full_name, "-opt1", val1, ...})`)方式に変更した。生成後に再設定可能な通常のオプションは、生成時に埋め込んでも最終的な状態は変わらないため、既存の全ウィジェットの動作に影響は無い。`config(options)`を呼んでいた約35個の具象ウィジェットクラス(classic/ttk合わせて)のコンストラクタを機械的に「基底コンストラクタへoptionsを渡す」形へ移行した(`Toplevel`のように追加ロジックを持つものは個別対応)。
  ...
  - [test/test_construction_time_options.cpp](../test/test_construction_time_options.cpp)で`ttk::PanedWindow`の`-orient`、`Frame`の`-class`/`-container`、`ttk::Separator`の`-orient`が構築時に正しく適用されることを回帰テスト化。[test/test_new_classes.cpp](../test/test_new_classes.cpp)の`ttk::PanedWindow`テストも、既定値のままにする回避策から`{{"orient", "horizontal"}}`を実際に指定する形に更新した。
...
**テストに関する既知の注意点（今回の実装時に判明）**: `xview`/`yview`呼び出し直後に`-xscrollcommand`/`-yscrollcommand`コールバックが同期的に発火するかはウィジェット・プラットフォーム実装依存で不安定だった（同じ手順でclassic Entryは発火する場合があるがttk::Entryでは発火しない、classic EntryとttkEntryでカーソル追従による自動スクロール巻き戻りの挙動も異なる、等）。そのため[test/test_high_impact_parity_gaps.cpp](../test/test_high_impact_parity_gaps.cpp)ではコールバックの発火有無ではなく、(1)`xview`/`yview`呼び出しが例外にならないこと、(2)`cget("xscrollcommand"/"yscrollcommand")`が正しく設定されること、の2点で検証している(Listboxの`yview`のみ実際の表示位置変化も検証できたため追加で確認済み)。

B/C/D区分は[test/test_low_priority_parity_gaps.cpp](../test/test_low_priority_parity_gaps.cpp)(14 TEST_CASE、44 assertion)、A区分は[test/test_high_impact_parity_gaps.cpp](../test/test_high_impact_parity_gaps.cpp)(6 TEST_CASE、16 assertion)、simpledialogは[test/test_simpledialog_return.cpp](../test/test_simpledialog_return.cpp)/[test/test_simpledialog_escape.cpp](../test/test_simpledialog_escape.cpp)で回帰テスト化した。これで第5回棚卸しのA〜D区分は全て対応済み。
...
B/C/D区分は[test/test_low_priority_parity_gaps.cpp](../test/test_low_priority_parity_gaps.cpp)(14 TEST_CASE、44 assertion)、および[test/test_simpledialog_return.cpp](../test/test_simpledialog_return.cpp)/[test/test_simpledialog_escape.cpp](../test/test_simpledialog_escape.cpp)で回帰テスト化した(A区分の対応・テストは上記の通り別途完了)。
...
7. **`tk::Tk`がInterpreter生成の唯一の入口になっていた（2026-07-05対応済み、破壊的変更）**
   ...
   - **テストへの影響（同一プロセス内の状態共有）**: `tk::Tk`が使い回されるようになったことで、1つのテスト実行ファイル（doctestは全TEST_CASEを同一プロセスで順に実行する）内の複数TEST_CASEが同じ"."ウィンドウ・同じInterpreterを共有するようになった。これにより、それまで「`tk::Tk`は毎回まっさらな新しいInterpreterを作る」という旧実装の副作用に暗黙で依存していた一部のテストが顕在化して壊れた。
     - `wait_visibility()`（Tclの`tkwait visibility`）は状態の**変化**を待つ命令であり、先行するTEST_CASEが既にmapped状態にしていると`deiconify()`だけでは新たな変化が起きず永久にブロックする（`test_native_misc.cpp`のwait_visibilityテスト自身、`test_low_priority_parity_gaps.cpp`のgrab/winfo系2箇所がタイムアウトしていた）。対応として、各該当TEST_CASEの冒頭で`root.withdraw(); root.deiconify();`という順で明示的に一度unmapしてから戻すことで、どの状態から始まっても確実に変化を起こすようにした。
...
- **A節（OptionMenu/LabelFrame/Menu/Listbox/Textの追加メソッド等）のテスト**: 記載当時は未着手だったが、その後の第2〜5回棚卸しで`test_tkinter_parity_batch1.cpp`/`test_tkinter_parity_batch2.cpp`/`test_new_classes.cpp`/`test_native_misc.cpp`/`test_high_impact_parity_gaps.cpp`/`test_low_priority_parity_gaps.cpp`を通じて大部分が回帰テスト化された。`filedialog`/`messagebox`は実際にOSのダイアログを表示し操作をブロックするため、引き続き自動テストでは呼び出しを避け手動確認に留めている。
...
## H. Tcl/Tk本体のランタイムスクリプト一式が実行環境に無いと起動できない問題（2026-07-05起票、2026-07-08部分対応済み）
...
### 方針決定・実装完了（2026-07-08）
ユーザと相談の結果、以下の方針で合意し実装した。

- **Tcl/Tk本体の自前ビルド(FetchContent/ExternalProject)はスコープ外**とする。Tcl/Tkは独自の
  configure/make(Windows側はnmake)ベースのビルドで、CMakeネイティブではないため、取り込むと
  プラットフォームごとのビルド手順を丸ごと肩代わりする規模の投資になり、READMEが明言する
  「枯れた基盤に薄いラッパーを被せるだけ」という設計方針と衝突する。`find_package`で見つけた
  スクリプトディレクトリをそのままコピーすれば、バージョン同期の懸念(同じインストール元のため
  必然的に一致する)も自前ビルドなしで解決できる。
- **zipfs(仮想ファイルシステム)によるVFS埋め込みは、対応時のみ使えるオプトイン機能として
  位置づけ、検出のみ実装する。** 実機検証の結果、このマシンのMSYS2 `mingw-w64-x86_64-tcl`/`tk`
  8.6.13は`tcl.h`に`Zipfs`関連のC API宣言が一切無く、`zipfs`Tclコマンドも未定義であることを
  確認した(`pacman -Ss tcl9`でもTcl 9系パッケージはMSYS2リポジトリに存在しない)。この開発環境
  ではzipfsの実行時コードを一切コンパイル・検証できないため、実際のzip化+VFSマウントの実装は
  見送り、検出結果(`CPP_TK_RUNTIME_HAS_ZIPFS`)を出すところまでに留めた。

#### 実装内容
1. **`cpp_tk::set_runtime_library_paths(tcl_library, tk_library)`**([cpp_tk.hpp](../cpp_tk.hpp)/
   [cpp_tk.cpp](../cpp_tk.cpp)): `Tcl_Init()`/`Tk_Init()`が探索するスクリプトディレクトリを明示
   指定するオーバーライドAPI。実装前に、`Tcl_CreateInterp()`後・`Tcl_Init()`前に`tcl_library`変数を
   `TCL_GLOBAL_ONLY`でセットしておくと、`TCL_LIBRARY`環境変数を一切設定していない状態でも、
   別名にリネームしたコピーディレクトリから正しく初期化できることを、生のTcl/Tk C APIを使った
   スクレッチプログラムで実機検証してから組み込んだ。`Interpreter::Interpreter()`(cpp_tk.cpp)で
   `Tcl_Init`/`Tk_Init`の直前にこの変数をセットするよう変更。呼び出しスレッドで最初にcpp_tkの
   オブジェクトを構築するより前に呼ぶ必要がある(Interpreterは遅延生成の1スレッド1個の
   シングルトンのため)。
2. **`cmake/CppTkRuntime.cmake`**(新規): `find_package(TCL)`が検出した実際のインストール
   (`TCL_LIBRARY`の格納先)と同じ`<prefix>/bin`から`wish`を優先的に探し(見つからなければ通常の
   PATH探索にフォールバック)、そのwishに`wm withdraw .; puts $tcl_library; puts $tk_library;
   puts [llength [info commands ::zipfs]]`という一時スクリプトを実行させることで、実際に使われる
   `$tcl_library`/`$tk_library`とzipfs対応有無を検出する(`CPP_TK_RUNTIME_TCL_LIBRARY`/
   `TK_LIBRARY`/`CPP_TK_RUNTIME_HAS_ZIPFS`)。
   - **注意点(実装中に発見)**: 単純に`find_program(... NAMES wish)`とすると、PATH上に複数の
     Tcl/Tkインストールがある環境(このマシンではGit for Windows同梱のmingw64が`find_package`
     が検出したMSYS2版より先にPATHへ出てくる)で、検出したライブラリと食い違うwishを掴んでしまい、
     コピーするスクリプトのバージョンがリンク先のlibtcl/libtkとズレる恐れがあることが分かった。
     `TCL_LIBRARY`から導出した prefix の`bin`を`HINTS`+`NO_DEFAULT_PATH`で優先することで解消した。
   - 再利用可能な関数`cpp_tk_package_runtime_scripts(<dest_dir>)`も定義し、検出したディレクトリを
     configure時に`<dest_dir>/tclX.Y`・`<dest_dir>/tkX.Y`としてコピーする。
3. **`CPP_TK_PACKAGE_RUNTIME_SCRIPTS`オプション**([CMakeLists.txt](../CMakeLists.txt)): ONにすると
   `cpp_tk_package_runtime_scripts(${CMAKE_RUNTIME_OUTPUT_DIRECTORY})`を呼び出し、実行ファイルと
   同じ場所にスクリプト一式をコピーする。ユーザ判断で既定を**ON**に変更し、実行ファイルの出力先も
   `${PROJECT_SOURCE_DIR}`直下から`${PROJECT_SOURCE_DIR}/bin`にまとめた(`bin/sample1.exe`と
   `bin/tcl8.6/`・`bin/tk8.6/`が同じ場所に並ぶことを目視確認済み)。コピー先はビルド成果物のため
   `.gitignore`に`/bin/*`を追加した。READMEのサンプル実行コマンドも`./bin/sample1`等に修正した。
4. **`test/test_runtime_library_override.cpp`**(新規回帰テスト): CMakeが検出した
   `CPP_TK_RUNTIME_TCL_LIBRARY`/`TK_LIBRARY`を、システム本来の場所とは別のビルドツリー内
   フィクスチャ(`test/runtime_fixture/`)へ複製し、`set_runtime_library_paths()`でそのフィクスチャ
   だけを指定して`tk::Tk`を構築、`Tcl_Init`/`Tk_Init`が成功することを確認する(検出できなかった
   環境ではテスト本体をスキップする)。[test/CMakeLists.txt](../test/CMakeLists.txt)の共通foreachとは
   別枠で追加し、`target_compile_definitions`経由でフィクスチャパスを受け渡す(当初`configure_file`で
   生成ヘッダを作る方式にしていたが、1テストが使う値3つのためだけにファイルを1つ増やす必要は無いと
   判断し、`target_compile_definitions`に置き換えた)。
5. リビルド・全26テスト(既存25+新規1)がgreenであることを確認済み。

#### 追加検証で発覚した不備とその修正（2026-07-08）
ユーザから「任意のパスで指定できていることを確認できているか」と問われ、検証し直した結果、
**当初のテストは検証として不十分だった**ことが判明した。`CPP_TK_PACKAGE_RUNTIME_SCRIPTS`が既定ONに
なったことで実行ファイルと同じ`bin/`に本物の`tcl8.6`/`tk8.6`が並ぶため、`set_runtime_library_paths()`
が全く機能していなくても、Tcl自身の相対パス探索等でたまたま初期化が成功してしまい得る状況だった。
実際に**存在しないパス**を指定して`tk::Tk`を構築する否定側の検証を追加したところ、**例外が送出されず
「成功」してしまう**という実バグを発見した。原因は`Interpreter`のコンストラクタが`Tcl_Init()`/
`Tk_Init()`の戻り値を一切チェックしておらず、失敗を握りつぶしたまま壊れたインタプリタで処理を
継続していたため(生のTcl C APIでは同条件で`Tcl_Init()`が正しく`TCL_ERROR`を返すことを別途確認し、
cpp_tk側の不備と特定した)。

- `Interpreter::Interpreter()`(cpp_tk.cpp)を修正し、`Tcl_Init()`/`Tk_Init()`の戻り値が`TCL_OK`で
  なければ`Tcl_DeleteInterp()`で後始末した上で`cpp_tk::Error`(Tcl側のエラーメッセージ付き)を送出する
  ようにした。これは`set_runtime_library_paths()`固有の問題ではなく、システムのTcl/Tk自体が壊れて
  いる場合等、従来から潜在していた一般的な欠陥でもある。
- `test/test_runtime_library_override.cpp`に否定側のTEST_CASEを追加し(存在しないパス→
  `CHECK_THROWS_AS(tk::Tk(), tk::Error)`)、肯定側(実在するがシステム既定とは別の場所への複製→
  成功)と対にすることで、オーバーライドが実際に効いていることを同一プロセス内で確認できるようにした。
- 修正後、否定側・肯定側とも期待通りの結果になること、全26テストがgreenのままであることを再確認済み。

#### 見送った内容(将来課題)
- zipfsによるzip化+VFSマウントの実行時コード自体は書いていない。zipfs対応のTcl/Tk(Tcl 9系、
  または`--enable-zipfs`でビルドされた8.6)が実機検証可能になった時点で、`CPP_TK_RUNTIME_HAS_ZIPFS`
  が`ON`の場合のみ有効化するオプトイン機能として別途着手する。

---

## I. 親と同じ具象型のWidgetを1引数で渡すと、コピー禁止によりコンパイルエラーになる（2026-07-06対応済み、2026-07-08追記）
...
### フリー関数からWidgetのメンバ関数への変更（2026-07-08）
ユーザから、「Widgetが自身を親として扱う」という意味であればメンバ関数として実装する方が自然ではないかとの相談を受けた。フリー関数のままで問題ないかを検証するため、実際に`const ArgValue&`シグネチャへの変更可否を検討する過程で確認した「一度に暗黙のユーザー定義変換は1回まで」という規則とは別問題であり、`as_parent()`が回避しているのは**コンストラクタのオーバーロード解決における暗黙コピーコンストラクタの完全一致優先**であって、これは戻り値の静的型が`const Widget&`でありさえすれば、呼び出し方がフリー関数かメンバ関数かに関係なく成立することを確認した。

- `Widget`クラス定義内(`full_name()`直後)に`const Widget& as_parent() const { return *this; }`を追加し、`cpp_tk.hpp`のWidgetクラス定義直後にあった同名のフリー関数を削除した。
- 呼び出し側3箇所(`custom.cpp`の`Calendar`実装、`example/theme/sv_ttk_demo.cpp`、`test/test_widget_basics.cpp`)を`tk::as_parent(w)`から`w.as_parent()`に置き換えた。
- README.mdの使用例コードも`container.as_parent()`に更新した。
- フリー関数を残す(後方互換のための再エクスポート等)ことはせず、完全に置き換えた(このライブラリはまだ1.0未満で、フリー関数版の外部利用実績もないため)。
- リビルド・全25テストがグリーンであることを確認済み。
...
### CIハングの原因と対応
`develop`ブランチ運用への切替後、実際にCIを走らせたところ、Linux版`test_low_priority_parity_gaps`の"Canvas::postscript"テストケースがSIGSEGVでクラッシュした後、`test_simpledialog_return`付近で**CIジョブ自体が10分以上進捗せず完了しない**という報告があった。
...
## L. ビルドスクリプト・テストコードの日本語コメントを英語化し、意図の分かりにくいテストファイル名を改名（2026-07-08対応済み）

### 経緯
`test_runtime_paths.hpp.in`を導入した際、ユーザから「1テストが使う値3つのためだけにファイルを増やす必要があるか」と指摘され、`target_compile_definitions`に置き換えて削除した。その流れで、ビルドスクリプト・テストコードの可読性・保守性についてユーザから2点要望があった: (1)テストファイル名を意図の分かりやすい名前に改名したい、(2)ビルドスクリプト・テストコード中の日本語コメントを全て英語表記に修正し、コンソール出力(テスト結果・ビルドログ)の文字化けを回避したい(対象は`CMakeLists.txt`/`cmake/*.cmake`/`test/*.cpp`・`*.hpp`。`cpp_tk.hpp`/`cpp_tk.cpp`本体・`docs/*.md`・`README.md`は対象外)。

### 対応内容
1. **テストファイルの改名**(`git mv`で履歴を保持): 過去の棚卸し回数・区分文字(A/B/C/D、round5等)に由来する不透明な名前だったものを、内容が分かる名前に変更した。
   - `test_a_features.cpp` → `test_tkinter_parity_batch1.cpp`
   - `test_a_features_round2.cpp` → `test_tkinter_parity_batch2.cpp`
   - `test_round5_a.cpp` → `test_high_impact_parity_gaps.cpp`
   - `test_round5_bcd.cpp` → `test_low_priority_parity_gaps.cpp`
   - `test_structural_fixes.cpp` → `test_construction_time_options.cpp`
   - 他13ファイル(`test_error_policy.cpp`等)は元々内容が分かる名前だったため変更していない。
   - `test/CMakeLists.txt`の`foreach`リスト、および本ドキュメント・`docs/todo.md`内の旧ファイル名への参照を全て更新した。
