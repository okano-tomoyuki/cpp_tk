...
- `ArgValue`はJSON的に配列/辞書を任意にネストできる合成値（`tk::list()`/`tk::dict()`、`-filetypes`等の入れ子構造も表現可能）
- `cpp_tk::Error`/`ErrorPolicy`による例外ベースのエラーハンドリング（未初期化アクセスや不正なoption名を検知可能）
- doctestベースの自動テスト（26件）とGitHub Actions CI（Windows/Linux、AddressSanitizer/UndefinedBehaviorSanitizerによる検証込み）
...
`tk::list()`で`-filetypes`のような入れ子構造を組む例:
...
int main()
{
    ...
    tk::Tk root;
    auto path = tk::filedialog::askopenfile({{"filetypes", tk::list({
...
## 📦 配布（Tcl/Tkランタイムスクリプトの同梱）

`Tcl_Init()`/`Tk_Init()`は、コンパイル済みライブラリとは別に`init.tcl`/`tk.tcl`等のTclスクリプト形式の
ランタイム一式(150〜250ファイル規模)を実行時に必要とします。これがシステムに無い環境(Tcl/Tk未
...
- **`cpp_tk::set_runtime_library_paths()`**: 配布先での格納場所をアプリ側で明示したい場合の
  ...
  ```cpp
  tk::set_runtime_library_paths("path/to/tcl8.6", "path/to/tk8.6");
  tk::Tk root; // ここでTcl_Init()/Tk_Init()が上記パスから初期化される
  ```
